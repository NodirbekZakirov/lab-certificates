...
import { api, type EquipmentDetail, type UserData } from '@/lib/api-client';
...
export default function EquipmentDetailPage({
  ...
  const handleDelete = async () => {
    if (!window.confirm('Вы уверены, что хотите удалить этот прибор? Действие необратимо.')) return;
  ...
  return (
    <div className="animate-fade-in pb-28">
      ...
      <div className="px-5 space-y-5 max-w-2xl mx-auto">
        <div className="glass-card p-6 shadow-md animate-slide-up relative overflow-hidden">
          {/* Equipment Photo as background with gradient or top section */}
          {equipment.photoUrl && (
            <div className="absolute top-0 left-0 right-0 h-32 w-full opacity-40 mix-blend-overlay">
              <img src={equipment.photoUrl} alt="equipment" className="w-full h-full object-cover" />
            </div>
          )}
          ...
          <div className="relative z-10">
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-center gap-4">
                {equipment.photoUrl && (
                  <img src={equipment.photoUrl} alt={equipment.name} className="w-16 h-16 rounded-full object-cover border-4 border-bg-card shadow-sm" />
                )}
                <h1 className="text-xl font-bold text-text-primary leading-snug flex-1 tracking-tight">
                  {equipment.name}
                </h1>
              </div>
            </div>
        ...
        {/* Update and Delete buttons (Admin only) */}
        {user?.role === 'admin' && (
          <div className="animate-slide-up delay-100 space-y-3">
            ...
            <button
              ...
              disabled={deleting}
              className="w-full p-4 rounded-2xl font-bold bg-status-red/10 text-status-red hover:bg-status-red hover:text-white transition-all flex items-center justify-center gap-2 border border-status-red/30 shadow-sm disabled:opacity-50"
            >
              {deleting ? (
